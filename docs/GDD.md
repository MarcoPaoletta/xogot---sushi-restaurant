...
**Document version:** 5.0 — 2026-09-08 (1.0 pre-build specification; 1.1 implementation notes; 2.0 the moving-chef redesign in section 15; 3.0 the readability pass in section 16; 3.1 desktop fullscreen notes; 4.0 the growing restaurant in section 17; 5.0 the UI and depth pass in section 18)
...
- **Recipes are visible, without text.** Each order bubble shows the recipe as a row of small spinning ingredient models under the dish. A collapsible recipe book (the **≡** button under the pause button) lists every dish of the day as `[dish] = [ingredient] + [ingredient]` using icons rendered from the real models: an offscreen 192 × 192 `SubViewport` in the HUD (`IconViewport`, own world, transparent background, studio environment) bakes each model once per session into a texture (`icon_baker.gd`, models fitted to a unit cube). The only text in the book is the `=` and `+` symbols.
- **Room for the chef.** The customer counter moved forward to Z 1.2 (stools and seats at Z 3.5, pass at Z 1.75), so the corridor is 2.6 u wide (Z −2.3 … 0.3) and the panda at Z −1.0 never clips either counter. The chef is the plain `Panda` (no knife and pan), so the `Idle_Holding` / `Walk_Holding` poses show the carried stack, which is carried in the hands: the `Hands` node is a child of the model (Y 1.55, Z 1.25 in front of the chest) so it turns with the panda, ingredients at 1.3× stacked every 0.5 u, the mixed dish at 1.7×.
...
## 18. V5 — UI pass, depth instead of width

Marco's notes on V4: the recipe book broke with twelve or fourteen combinations ("max 7 different foods"), things clipped at the counter, the whole game happened in the top two thirds of the screen, and some lanterns floated.

- **Seven dishes a day, always.** Ingredients still arrive every day (4 → 6 → 8 → 9 → 9 stations) but the menu *rotates*: Days 1–3 build up to seven dishes, Day 4 keeps two favourites and brings the four Festival dishes, Day 5 is the seven late dishes (octopus, urchin, avocado, crab, eel, squid, mackerel). The recipe book is therefore one column of at most seven rows and always fits.
- **No more widening.** The room keeps its V3 width; the wide wall set, the extra counters and the fourteen-slot layout are gone. Stations use four slots left of the pass and five right (the fridge stands at the far left), still filled centre-first, so nothing sits beyond X 9.
- **The lower third.** The camera moved from (0, 11.5, 17.5) at −30° to (0, 13.0, 16.5) at −38° — higher, closer and tilted down, so the frame's top edge sits just above the sign instead of showing a quarter of bare wall, and the floor in front of the counter fills the lower third. From Day 4 that floor holds **tables**: two on Day 4, four on Day 5 (`Tables/TableN` with a chair and a `Seat` marker). A rabbit whose order is complete carries its dish from the stool to a free table, eats there and leaves from there, which frees the stool at once. Because stools no longer throttle arrivals, Days 4 and 5 space customers 6.5 s and 6.0 s apart with 32 s of patience (targets 200 and 240); the scripted tester, which serves one order at a time, completes both. Rabbits walk a lane 2.0 u in front of the stools, between the stools and the tables.
- **Clipping.** Stools and seat markers moved back to Z 3.9 so a seated rabbit's chest no longer passes through the counter lip; the drawers segment that stood proud of the counter front was swapped for a flush one; the wall cabinet, steamer and bottles were removed because they overlapped the centre stations' labels.
- **No sink.** The sink was a fourth interaction target at the right end of the counter and its floating label collided with the outermost station; it is gone. Binning moved to the pass: standing there with a finished dish nobody wants, the action reads *Bin it*. The station grid therefore uses the centre of the counter too — the innermost pair sits at X ±1.35 (the chef and the pass fill the 2.7 u gap between them), then ±3.15, ±4.95, ±6.75, ±8.55. A day with an odd number of stations drops the slot just left of the chef rather than hanging one extra station off the left end, so both ends of the row always sit level (a nine-station day runs −8.55 … 8.55).
- **A counter centred on the pass.** The back counter's five units sit at X −8, −4, 0, 4 and 8, so it spans −10 … 10 symmetrically around the pass and the outermost stations (±8.55) have wood under them — before this the row hung off the left end while three metres of counter sat empty on the right. A tall fridge anchors each end (X ±11), the same model at the same rotation, so the row is bookended symmetrically.
- **Ingredients fit themselves.** Each station scales its model so the longest edge is 0.85 u, stands it on the board (the bottom of its bounding box on the surface), turns its long axis along the counter and leans flat things (nori, a fillet, the eel) back 34°. The kit authors these models at wildly different scales and origins — the squid was three times the size of the rice and the eel was sunk into the wood — and this measures each one instead of hand-tuning fourteen numbers. The red oven, which clipped through the side wall, was removed.
- **The recipe book** sits in the bottom-right corner and opens upward from its ≡ button, or with the **R** key (input action `recipes`); on phones both lift 210 px to clear the on-screen action button.
- **The counter row.** Station labels sit on one line (Y 1.6 above the counter, 40 px Lilita instead of 56) so nothing overlaps whatever the day's mix of names is, and the two wall paintings moved up to Y 4.7 out of that line. Four warm omni lights (X ±2.5 and ±7.5, Y 4.0, Z −2.2, energy 2.2, range 9) light the whole back counter, which the sun left in the wall's shadow, and the room's ambient light rose from 0.7 to 1.05 so the ingredients read at a glance.
- **Lights.** The two standing lanterns are on the floor beside the counter ends (X ±11, Z 6.8); the back wall got two hanging 寿司 lanterns (`Decoration_WallLight`, Day 4) high enough (Y 5.4) to clear the outer labels.
