...
- **A counter centred on the pass.** The back counter's five units sit at X −8, −4, 0, 4 and 8, so it spans −10 … 10 symmetrically around the pass and the outermost stations (±8.55) have wood under them — before this the row hung off the left end while three metres of counter sat empty on the right. The tall fridge anchors the left end (X −11) and a drinks fridge the right (X 11).
...
- **The recipe book** sits in the bottom-right corner and opens upward from its ≡ button; on phones both lift 210 px to clear the on-screen action button.
